 ...
 * POST /api/whatsapp/pausar — pausa a IA pra um contato especifico (persistente)
 * POST /api/whatsapp/retomar — retoma a IA pra um contato especifico
...
router.get("/conversas", async (_req, res) => {
  try {
    ...
    // Fonte da verdade pra "IA pausada" — persistida no banco (ver
    // whatsapp_silenciados), não só na sessão em memória desta instância.
    const silenciados = await listarTelefonesSilenciados();
    ...
    const conversas = Object.entries(agrupado).map(([telefone, mensagens]) => {
      ...
      return {
        ...
        iaPausada: silenciados.has(telefone) || iaEstaPausada(telefone),
      };
    });
...
// Pausa persistente (banco de dados) — usada quando a Dra./equipe decide, pelo
// painel, que a IA deve parar de responder a um contato específico (ex: paciente
// insistindo com assunto fora do escopo do bot). Fica salva mesmo se o servidor
// reiniciar ou a próxima mensagem cair numa instância serverless diferente.
router.post("/pausar", async (req, res) => {
  const telefone = extrairTelefone(String(req.body?.telefone || ""));
  if (!telefone) return res.status(400).json({ ok: false, error: "telefone é obrigatório" });
  await pausarIaPermanente(telefone, req.body?.motivo);
...
router.post("/retomar", async (req, res) => {
  const telefone = extrairTelefone(String(req.body?.telefone || ""));
  if (!telefone) return res.status(400).json({ ok: false, error: "telefone é obrigatório" });
  await retomarIaPermanente(telefone);
