...
import { processarMensagem, formatarNotificacaoDra, MensagemConversa } from "../services/iaSecretaria";
import { enviarMensagem, enviarLinkPagamento, extrairTelefone, WebhookZAPI } from "../services/whatsappService";
import { criarOuBuscarCliente, gerarLinkPagamento, pagamentoConfirmado } from "../services/pagamentoService";
...
router.get("/stats", async (_req, res) => {
  try {
    ...
    const botOnline = !!(process.env.ZAPI_BASE_URL && process.env.ZAPI_TOKEN);
...
router.post("/config", (req, res) => {
  // Em produção, salvar no banco ou Vercel env via API
  // Aqui apenas confirmamos o recebimento (vars já definidas no Vercel)
  console.log("Config recebida:", Object.keys(req.body));
  res.json({ ok: true, message: "Configure as variáveis de ambiente no painel Vercel para persistência." });
});
...
router.get("/test-connection", async (_req, res) => {
  ...
  if (!zapiBase || !zapiToken) {
    return res.status(400).json({ ok: false, error: "Z-API não configurado" });
  }
  try {
    const r = await fetch(`${zapiBase}/status`, {
      headers: { "Client-Token": zapiToken }
    });
    if (r.ok) return res.json({ ok: true });
    return res.status(400).json({ ok: false, error: "Z-API offline" });
  } catch {
    return res.status(500).json({ ok: false, error: "Sem conexão com Z-API" });
  }
});
