...
export default function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  // Botão com efeito de brilho diagonal ao passar o mouse — clássico toque premium.
  const shineButton = (className: string, disabled: boolean | undefined, content: React.ReactNode) => (
    <button type="submit" disabled={disabled}
      className={`relative overflow-hidden isolate flex items-center justify-center gap-2 cursor-pointer mt-1 ${className}`}>
      <span className="pointer-events-none absolute inset-0 -translate-x-[120%] skew-x-[-18deg] bg-gradient-to-r from-transparent via-white/35 to-transparent transition-transform duration-700 ease-out group-hover:translate-x-[120%] hover:translate-x-[120%]" />
      <span className="relative z-10 flex items-center justify-center gap-2">{content}</span>
    </button>
  );

  return (
    <div className="min-h-screen bg-[#0A0A0A] lg:grid lg:grid-cols-2 relative">
      {/* Textura de grão sutil sobre toda a tela — evita o look "chapado" digital */}
      <div className="fixed inset-0 pointer-events-none z-0 opacity-[0.035] mix-blend-overlay" style={{ backgroundImage: GRAIN_URL }} />
      ...
      <div onMouseMove={handlePanelMouseMove}
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          ...
          <div className="relative w-11 h-11 flex items-center justify-center">
            <motion.div className="absolute inset-0 rounded-full border border-[#C9A84C]/40"
              animate={{ scale: [1, 1.35, 1], opacity: [0.5, 0, 0.5] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} />
            <div className="relative w-11 h-11 rounded-full border-2 border-[#C9A84C]/60 bg-black flex items-center justify-center shadow-[0_0_30px_rgba(201,168,76,0.2)]">
        ...
        <div className="w-full max-w-sm relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: -14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
            ...
            <div className="relative w-14 h-14 flex items-center justify-center mb-3">
              <motion.div className="absolute inset-0 rounded-full border border-[#C9A84C]/40"
                animate={{ scale: [1, 1.3, 1], opacity: [0.5, 0, 0.5] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} />
              <div className="relative w-14 h-14 rounded-full border-2 border-[#C9A84C]/60 bg-black flex items-center justify-center shadow-[0_0_30px_rgba(201,168,76,0.15)]">
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.1 }}
            className="relative">
            {/* cartão de vidro com cantos em fio dourado — moldura discreta de "recepção" */}
            <div className="relative rounded-2xl bg-white/[0.02] backdrop-blur-xl border border-white/[0.06] p-6 sm:p-7">
                    ...
                    <AnimatePresence mode="wait">
                      {mainRole === "medica" ? (
                        <motion.form key="medica" initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: 0.18 }}
                            ...
                            {fieldWrap(
                              <div className="relative">
                                ...
                                <input type="email" value={email} onChange={e => { setEmail(e.target.value); setErrMedica(""); }} autoFocus
                                  ...
                                  className="peer w-full bg-[#0A0A0A] border border-[#2A2A2A] focus:border-[#C9A84C] focus:ring-2 focus:ring-[#C9A84C]/15 text-white text-sm py-3 pl-10 pr-4 rounded-lg outline-none transition-all placeholder:text-neutral-700" />
                              </div>, "#C9A84C"
                            )}
                          </div>
                          <div>
                            ...
                            {fieldWrap(
                              <div className="relative">
                                ...
                                <input type="password" value={senha} onChange={e => { setSenha(e.target.value); setErrMedica(""); }}
                                  placeholder="••••••••"
                                  className="peer w-full bg-[#0A0A0A] border border-[#2A2A2A] focus:border-[#C9A84C] focus:ring-2 focus:ring-[#C9A84C]/15 text-white text-sm py-3 pl-10 pr-4 rounded-lg outline-none transition-all" />
                              </div>, "#C9A84C"
                            )}
                          </div>
                          ...
                          <div className="group">
                            {shineButton(
                              "w-full bg-gradient-to-r from-[#C9A84C] to-[#D9B85C] hover:from-[#D9B85C] hover:to-[#E9C86C] disabled:opacity-50 disabled:cursor-not-allowed text-black text-xs font-bold uppercase tracking-widest py-3.5 rounded-lg transition-all shadow-[0_4px_20px_rgba(201,168,76,0.15)] hover:shadow-[0_6px_28px_rgba(201,168,76,0.3)]",
                              loading,
                        ...
                        <motion.form key="paciente" initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: 0.18 }}
                            ...
                            {fieldWrap(
                              <div className="relative">
                                ...
                                <input type="text" value={cpf} onChange={e => { setCpf(formatCpf(e.target.value)); setErrPac(""); }} autoFocus
                                  ...
                                  className="peer w-full bg-[#0A0A0A] border border-[#2A2A2A] focus:border-[#6B9FD4] focus:ring-2 focus:ring-[#6B9FD4]/15 text-white text-sm py-3 pl-10 pr-4 rounded-lg outline-none transition-all placeholder:text-neutral-700 font-mono tracking-wider" />
                              </div>, "#6B9FD4"
                            )}
                          </div>
                          ...
                          <div className="group">
                            {shineButton(
                              "w-full bg-gradient-to-r from-[#6B9FD4] to-[#7BAEE4] hover:from-[#7BAEE4] hover:to-[#8BBEF4] disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-bold uppercase tracking-widest py-3.5 rounded-lg transition-all shadow-[0_4px_20px_rgba(107,159,212,0.15)] hover:shadow-[0_6px_28px_rgba(107,159,212,0.3)]",
                        ...
                        {fieldWrap(
                          <div className="relative">
                            ...
                            <input type="password" value={pin} onChange={e => { setPin(e.target.value); setErrDev(""); }} autoFocus
                              placeholder="••••••••"
                              className="peer w-full bg-[#0A0A0A] border border-[#2A2A2A] focus:border-[#9B8EAF] focus:ring-2 focus:ring-[#9B8EAF]/15 text-white text-sm py-3 pl-10 pr-4 rounded-lg outline-none transition-all font-mono tracking-widest" />
                          </div>, "#9B8EAF"
                        )}
                      </div>
